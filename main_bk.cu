...
// matrix [vocab, batch]
// <<<vocab/256,256>>>
__global__ void dense2array_compact(float *matrix, int vocab_size, int batch_size, int* index_array, int topn, int threshold, int *n){
...
int main() {
  ...
  { //dense2array
    ...
    h_rowIdx = (int *) malloc(voc*sizeof(int));
    cudaMalloc((void **)&d_rowIdx, voc*sizeof(int));
    cudaMalloc(&d_new_db_buf, voc*embed*sizeof(float));
   
    for (int i = 0; i < topn; i ++){
      h_rowIdx[i] = i;
    }
    cudaMemcpy(d_rowIdx,h_rowIdx,topn*sizeof(int),cudaMemcpyHostToDevice);

    cudaMemcpy(d_new_db_buf,d_db_buf,topn*embed*sizeof(float),cudaMemcpyDeviceToDevice);
    ...
    for (int i = 0; i < Test; i++) {
      ...
      nnz = std::floor(fnnz);
      cudaMemset(d_rowIdx, 0, voc * sizeof(int));
      compact(h_input,h_output,d_index_buf,d_rowIdx,voc);
      fill_new_db<<<dim3((nnz + thread_y-1)/thread_y),dim3(thread_x,thread_y)>>>((float *)d_new_db_buf, (float *)d_db_buf,d_rowIdx,embed, nnz, 0); // 0.84 ms
      } else {
      h_nnz[0] = topn;
      cudaMemcpy(d_nnz, h_nnz, sizeof(int), cudaMemcpyHostToDevice);
      dense2array_compact<<<(voc-topn+255)/256, 256>>>((float*)d_dist_buf_input, voc, batch, (int *)d_rowIdx, topn,threshold, d_nnz); //0.02ms     
      cudaMemcpy(h_nnz, d_nnz, sizeof(int), cudaMemcpyDeviceToHost);
      nnz = h_nnz[0];


      fill_new_db<<<dim3((nnz - topn + thread_y-1)/thread_y),dim3(thread_x,thread_y)>>>((float *)d_new_db_buf, (float *)d_db_buf,d_rowIdx,embed, nnz, topn); // 0.84 ms
      
      }
      
      

      //cudaMemcpy(d_test_in, d_test_out, 100000 * embed * sizeof(float), cudaMemcpyDeviceToDevice);
      ...
      array2dense<<<dim3((nnz+1024-1)/1024,batch), 1024>>>((float*)d_dist_buf_1,(float*)d_dist_buf_2,d_rowIdx,voc,nnz);
    ...
    std::cout << "d_rowIdx\n";
    print_matrix_gpu(d_rowIdx,  voc, 1,   offset+0, offset+10, 0, 1 );

    std::cout << "d_db_buf\n";
    print_matrix_gpu((float*)d_db_buf, embed, voc, 0,10,offset+0, offset+10 );

    std::cout << "d_new_db_buf\n";
    print_matrix_gpu((float *)d_new_db_buf,  embed, nnz, 0,10,offset+0, offset+10);
    ...
    print_matrix_gpu((float *)d_dist_buf_1, voc, batch, 0, voc, 0,batch );
